...
//            .receive(on: RunLoop.main)
    ...
    func testObjectScheduler() {
        let obj = try! realm.write { realm.create(SwiftIntObject.self, value: []) }

        let queue = DispatchQueue(label: "scheduler queue")
    ...
    func testReceiveOn2() {
        ...
        token = RealmPublisher(obj)
            ...
            .receive(on: DispatchQueue(label: "queue"), options: .init(group: group))
            .compactMap { try! Realm(configuration: Realm.Configuration(inMemoryIdentifier: "test")).resolve($0) }
            .print()
            .map { obj -> Int in
                exp.fulfill()
                return obj.intCol
